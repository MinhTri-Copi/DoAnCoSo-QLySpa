...
@section('title', 'Sửa Phiếu Hỗ Trợ')
...
<div class="container-fluid">
    <div class="support-form-card">
        <div class="support-form-header">
            <h1 class="support-form-title">Sửa Phiếu Hỗ Trợ</h1>
        ...
        <form method="POST" action="{{ route('admin.phieuhotro.update', $phieuHoTro->MaphieuHT) }}" class="support-form-body">
            @csrf
            @method('PUT')
            <div class="form-group">
                <label for="MaphieuHT" class="form-label">Mã Phiếu Hỗ Trợ</label>
                <input id="MaphieuHT" type="number" class="form-control @error('MaphieuHT') is-invalid @enderror" name="MaphieuHT" value="{{ $phieuHoTro->MaphieuHT }}" required readonly>
                @error('MaphieuHT')
                    <div class="invalid-feedback">{{ $message }}</div>
                @enderror
                <small class="text-muted">Mã phiếu không thể thay đổi</small>
            </div>

            <div class="form-group">
                <label for="Noidungyeucau" class="form-label form-required">Nội Dung Yêu Cầu</label>
                <textarea id="Noidungyeucau" class="form-control @error('Noidungyeucau') is-invalid @enderror" name="Noidungyeucau" required>{{ $phieuHoTro->Noidungyeucau }}</textarea>
                @error('Noidungyeucau')
                    <div class="invalid-feedback">{{ $message }}</div>
                @enderror
            ...
            <div class="form-group">
                ...
                <select id="Matrangthai" class="form-select @error('Matrangthai') is-invalid @enderror" name="Matrangthai" required>
                    ...
                    @foreach ($trangThais as $trangThai)
                        <option value="{{ $trangThai->Matrangthai }}" {{ $phieuHoTro->Matrangthai == $trangThai->Matrangthai ? 'selected' : '' }}>{{ $trangThai->Tentrangthai }}</option>
                    @endforeach
                </select>
                @error('Matrangthai')
                    <div class="invalid-feedback">{{ $message }}</div>
            ...
            <div class="form-group">
                ...
                <select id="Manguoidung" class="form-select @error('Manguoidung') is-invalid @enderror" name="Manguoidung" required>
                    ...
                    @foreach ($users as $user)
                        <option value="{{ $user->Manguoidung }}" {{ $phieuHoTro->Manguoidung == $user->Manguoidung ? 'selected' : '' }}>{{ $user->Hoten }}</option>
                    @endforeach
                </select>
                @error('Manguoidung')
                    <div class="invalid-feedback">{{ $message }}</div>
            ...
            <div class="form-group">
                ...
                <select id="MaPTHT" class="form-select @error('MaPTHT') is-invalid @enderror" name="MaPTHT" required>
                    ...
                    @foreach ($pthotros as $pthotro)
                        <option value="{{ $pthotro->MaPTHT }}" {{ $phieuHoTro->MaPTHT == $pthotro->MaPTHT ? 'selected' : '' }}>{{ $pthotro->TenPT }}</option>
                    @endforeach
                </select>
                @error('MaPTHT')
                    <div class="invalid-feedback">{{ $message }}</div>
