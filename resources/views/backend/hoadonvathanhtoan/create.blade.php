...
<div class="container">
    ...
    <form action="{{ route('admin.hoadonvathanhtoan.store') }}" method="POST">
        @csrf
        <div class="mb-3">
            <label for="MaHD" class="form-label">Mã hóa đơn</label>
            <input type="text" class="form-control" id="MaHD" name="MaHD" value="{{ $suggestedMaHD }}" readonly>
        </div>
        <div class="mb-3">
            <label for="Ngaythanhtoan" class="form-label">Ngày thanh toán</label>
            <input type="date" class="form-control" id="Ngaythanhtoan" name="Ngaythanhtoan" value="{{ old('Ngaythanhtoan') }}">
        </div>
        <div class="mb-3">
            <label for="Tongtien" class="form-label">Tổng tiền (VNĐ)</label>
            <input type="number" class="form-control" id="Tongtien" name="Tongtien" value="{{ old('Tongtien') }}">
        </div>
        <div class="mb-3">
            <label for="MaDL" class="form-label">Mã đặt lịch</label>
            <select class="form-control" id="MaDL" name="MaDL">
                @foreach ($datLichs as $datLich)
                    <option value="{{ $datLich->MaDL }}" {{ old('MaDL') == $datLich->MaDL ? 'selected' : '' }}>
                        {{ $datLich->MaDL }}
                    </option>
                @endforeach
            </select>
